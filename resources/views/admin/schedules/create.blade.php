...
@section('seo-title') 
<title>Create course & teacher {{ config('app.seo-separator')}}{{ config('app.name')}}</title>
@endsection 
...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Create course & teacher </h1>
    </div>   
    <!-- /.col-lg-12 -->
            ...
            <form role="form" method="post" action="">
                    ...
                    <select class="form-control" name="teachers_id">
                        ...
                        @if(count($teachers)>0)
                            @foreach($teachers as $value)
                               <option value="{{$value->id}}" @if(old('teachers_id')==$value->id) selected  @endif>{{$value->name}} {{$value->surname}}</option>
                            @endforeach
                        @endif
                    </select>
                ...
                <div class="form-group{{$errors->has('start_date')?' has-error':''}}">
                    ...
                    <div class='input-group date'>
                        <input id="date" name="start_date" placeholder="Year / Month / Day" value="{{old('start_date')}}" type='text' class="form-control" />
                ...
                <div class="form-group{{$errors->has('end_date')?' has-error':''}}">
                    ...
                    <div class='input-group date'>
                        <input id="date" name="end_date" placeholder="Year / Month / Day" value="{{old('end_date')}}" type='text' class="form-control" />
                        <span class="input-group-addon">
                            <span class="glyphicon glyphicon-calendar"></span>
                        </span>
                    </div>
